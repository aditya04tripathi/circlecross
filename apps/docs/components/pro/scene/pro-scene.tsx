  ...
  return (
    <group ref={group} position={[0.85, -0.05, 0]}>
      {rings.map((ring) => (
        <lineLoop key={ring.key} geometry={ring.geometry}>
          <lineBasicMaterial
            ...
            transparent
            opacity={ring.accent ? 0.42 : 0.18}
          />
        </lineLoop>
      ))}

      {/* Diminished Central Professional Core */}
      <mesh>
      ...
      <lineSegments ref={linksRef} geometry={linkGeometry}>
        <lineBasicMaterial color="#8688ab" transparent opacity={0.22} />
      </lineSegments>

      {/* Diminished Relationship Graph Nodes */}
      {nodes.map((n) => (
...
export function ProScene({ progress, active }: ProSceneProps) {
  return (
    ...
    >
      ...
      <directionalLight position={[-3, 4, 5]} intensity={3.8} color="#edf0ff" />
      <directionalLight position={[4, -2, 2]} intensity={2.0} color="#cd7c47" />
